...
/** Наблюдатель за изменениями DOM для отслеживания пропадания таблицы */
let MUTATION_OBSERVER = new MutationObserver(mutationCallback);

/**
 * Обработчик мутаций DOM - вызывается при изменении структуры страницы
...
/**
 * Начинает слежение за изменением элемента с названием журнала
 */
function StartWatch() {
...
function insertButton() {
    ...
    if (mainSection.length == 0) {
       ...
       .licey24_but:hover {
        ...
        // Кнопка принудительной покраски
        ...
        // Кнопка вывода информации о обработке
        ...
        bu2.addEventListener("click", () => createModalWindow());
        di.append(bu2);

        // Кнопка копирования в эксель
